...
const ChekoutUI = () => {
  ...
  const { saveShippingAdress, shippingAdress, totalPrice, shippingPrice,setShippingPrice, initialize, items } = useCartService();
...
<div>
      ...
      {selectedState!=""?(<><span>سعر التوصيل </span ><div className='text-xl'>{shippingPrice} DA</div></>):(<></>)}
    </div>
          <h2 className="text-2xl font-bold mb-4">السعر الإجمالي</h2>
          <p className="text-gray-700 text-lg mb-2">{totalPrice + shippingPrice} DA</p>
        </form>
        {selectedCity !== '' ? (
